...
def count_total_word(sentence_list):
    total_word = 0
    for sentence in tqdm(sentence_list, desc='count words'):
        for _ in sentence:
            total_word += 1
...
def unigramTable(vocabulary, word_to_id):
    ...
    for word in tqdm(vocabulary.keys(), desc='Making UnigramTable'):
        if word == '\s':
            continue
...
def code_to_id(codes, root, vocabulary):
    ...
    idx = []
    code_sign = []
    for word in tqdm(vocabulary, desc='get codes to index'):
        temp0 = []
        temp1 = []
        code = codes[word]
        for c in code:
...
def convert2vec(valid, word_vectors, word_to_id):
    ...
    d = []
    for s in tqdm(valid):
        a.append(word_vectors[word_to_id[s[0]]])
        b.append(word_vectors[word_to_id[s[1]]])
        c.append(word_vectors[word_to_id[s[2]]])
        d.append(word_vectors[word_to_id[s[3]]])
